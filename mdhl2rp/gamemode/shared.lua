...
mint = mint or {}
mint.lib = landis.lib or {}
mint.__VERSION = "DEV-0.1"
mint.__DISPLAY = "Amongus"
mint.__XTNOTES =  sharting
mint.__DVBUILD = true
...
mint.Config.ConsolePrefix    = "[mint]"
// instead of writing out the same LONG ASS FUCKING MESSAGE use this simple function!! :)))
function mint.ConsoleMessage(...)
	local mColor = mint.Config.MainColor
	local prefix = mint.Config.ConsolePrefix .. " "
	local textCo = mint.Config.DefaultTextColor
	MsgC(mColor,prefix,textCo,...,"\n") // \n to prevent same line console messages
end

function mint.lib.includeDir( scanDirectory, isGamemode )
